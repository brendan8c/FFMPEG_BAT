...
@REM *--------------------------*
@REM for %%a in ("Your_files\*.avi") do ffmpeg -y -i "%%a" -ss 00:00:00 -t 00:00:08 -c copy "Result\%%~na.avi"
...
@REM for %%a in ("Your_files\*.avi") do ffmpeg -y -to 00:00:08 -i "%%a" -map 0 -c copy "Result\%%~na.avi"
...
@REM for %%a in ("Your_files\*.avi") do ffmpeg -y -ss 00:00:00 -i "%%a" -to 00:00:08 -c copy -copyts "Result\%%~na.avi"
@REM *--------------------------------------------------------------------------------------------------------*
@REM Slower search. (Not more accurate as I noticed.)
@REM Более медленный поиск. (Не является более точным как я заметил.)
@REM for %%a in ("Your_files\*.avi") do ffmpeg -y -i "%%a" -ss 00:00:01 -to 00:00:08 -c copy "Result\%%~na.avi"
@REM *--------------------------------------------------------------------------------------------------------*
...
@REM for %%a in ("Your_files\*.avi") do ffmpeg -y -ss 00:00:00 -i "%%a" -frames:v 250 -c copy "Result\%%~na.avi"
